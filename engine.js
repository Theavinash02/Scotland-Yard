...
function newGame(seats){
  var nd=seats.length-1;
  return {mv:0,seats:seats,nd:nd,
    mrx:{st:rnd(MRX_STARTS),black:nd,dbl:2},
...
function startDouble(g){
  if(g.turn===-1&&g.mrx.dbl>0&&g.dblPending===0&&g.log.length<MAX_ROUND-1){g.mrx.dbl--;g.dblPending=2;return true;}
  return false;
}
function applyMrx(g,m){
  var from=g.mrx.st;
  g.mrx.st=m.to;if(m.tk==='x')g.mrx.black--;
  var round=g.log.length+1,rv=REVEALS.indexOf(round)>=0;
...
function normalizeTurn(g){
  ...
  if(g.log.length>=MAX_ROUND){g.winner='mrx';g.reason='The travel log is full — Mr. X slipped away after 24 rounds.';return;}
  g.turn=-1;
